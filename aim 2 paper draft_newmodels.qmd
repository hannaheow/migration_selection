...
# load("data_processed/migterm_imp.Rdata") 
# # this dataset has natality, death, population, premature mortality rates, migration flows, rural/urb codes and two different precalculated migterms (one including natality and one not including natality)
# 
# mnn = migterm_imp %>% 
#  arrange(year) %>% 
#  group_by(destid) %>% 
...
# mnn$ft = as.factor(mnn$t)
# 
# migpanel = plm::pdata.frame(mnn, index = c("destid", "year"))
# 
# mp1 = plm(rate_d1 ~ ft + migterm, model = "random", data = migpanel)
# msp1 = splm::spml(rate_d1 ~ ft + migterm, model = "random", data = migpanel)
# 
# 
...
Below is a table showing $mig_{it}$ and $y_{it}$ by year:

```{r table1 by year, echo = FALSE, warning = FALSE, message = FALSE}
table1::label(cpall$rate_d1) = "Premature age-adjusted mortality rate"
table1::label(cpall$migterm) = "Weighted average migration term"
table1::table1(~ rate_d1 + migterm | year, data = cpall %>% filter(year!="2011"))
```
...
We analyzed several models and used the Bayesian Information Criterion (BIC) to determine the best fitting model. BIC is commonly used to determine models with greatest explanatory power [@kuha2004]. Starting with our baseline model, we iteratively added splines for the prior year's average county-level premature mortality rate and the migration term, capturing potentially nonlinear trends. We chose to keep the number of degrees of freedom equal for splines for the migration term and premature mortality rate since we noticed that imbalanced degrees of freedom tended to produce higher BIC scores. The table below shows the model with the lowest BIC score in blue. All models were created using the splm and and splines R packages [@splines][@millo2023].
...
tablex = data.frame("Migration term included" = c(rep(TRUE, 5), rep(FALSE, 5)), 
                    "Degrees of freedom" = rep(c(1,2,3,4,5), 2),
                    "BIC" = c(bicnospat[1], bicnospat[3], bicnospat[5], bicnospat[7], bicnospat[9],
                              bicnospat[2], bicnospat[4], bicnospat[6], bicnospat[8], bicnospat[10]))


library(gt)
gt::gt(tablex) |> tab_style(style = list(cell_fill(color = "lightcyan"), 
                                      cell_text(weight = "bold")),
                            locations = cells_body(rows = (BIC == min(BIC)))) |>
  cols_label(Migration.term.included = "Migration included", Degrees.of.freedom="DF", BIC= "BIC") |>
  tab_header(title = "Models without spatial error")

```

**To test Hypothesis 2**, we built models that accounted for spatial error as well as migration again using the splm and spliin R[@millo2023]. We again compared these models using BIC while iteratively adding splines with equal degrees of freedom for migration and lagged premature mortality. Again, the model with the lowest BIC score is shown in blue in the table below.

```{r spatial models here, echo = FALSE, warning = FALSE, message = FALSE}
...
For a more robust comparison, we then accounted for spatial error. Again, we found that the best fitting model in terms of BIC score included splines with four degrees of freedom for both the autoregressive term and our weighted average migration term. Additionally, we determined that models that accounted for both spatial error and migration had lower BIC scores than models that did not account for spatial error or did not include our migration term.
