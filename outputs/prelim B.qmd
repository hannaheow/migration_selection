...
<!--# add params to subtract from rate_j,t-1 (a) and add to rate_i,t-1 (b); params depend on urb/rural of i and j ; add column a where has an urb value and a rural value; start with a and b independent - just pick a_urb, a_rur, b_urb, b_rur. what is relationship between a and b?? a and b together should somehow still average to rate_j,t-1 - ie a and b are constrained somewhow, maybe need function to explore their relationship  -->

Second, we calculate a "net migration" term which is essentially the sum of the total number of years between 2012 and 2019 that a county experienced positive net migration minus the total number of years that a county experienced negative net migration, as shown below.
...
pd %>% ggplot(aes(fill= included, x = pop)) +  
 ...
 # labs(fill = "Included in our analyses") +  
  labs(caption = "Figure 2: Log-transformed populations \n of included and excluded counties") + 
  theme(plot.caption = element_text(hjust =0))
...
```{r netmig plot, echo = FALSE, message = FALSE, warning = FALSE}
...
Despite the fact that net migration adds explanatory power to our model, from Figure 3 above, we see that there do no appear to be substantial rural/urban differences in net migration's effect on predicted mortality.

<!--# need more here if we decide not to exclude -->
