...
3.  The role that intercounty migration plays in county-level premature mortality differs significantly between rural and urban counties.
...
<!--# need to update this table and/or verify that it's doing what i think it's doing -->

```{r urban rural mod selection, echo = FALSE, message = FALSE, warning = FALSE}
#BIC scores were generated in "spatial mod selection.R"  
...
#minimum BIC: 
#chtctot[chtctot$bicspat == min(chtctot$bicspat, na.rm=TRUE),]


bicmin = chtctot %>% filter(kuu == 0 & krr == 0)
...
Then, to quantify potential rural-urban differences in the role of intercounty migration on county-level health, we used spatial error models to assess the effect of migration on premature age-adjusted mortality separately for rural and urban counties. We found that for both urban and rural counties, the models with the lowest BIC scores included splines with four degrees of freedom each for the migration term and lagged age-adjusted mortality as well as the net migration term factor variable. Therefore, **we cannot form strong conclusions about Hypothesis 3**: the role that intercounty migration plays in county-level premature mortality does not appear to differ significantly between rural and urban counties.

Finally, we simulated health-related selection for migration using $k_i$ and $k_j$ and concluded that, based on our models, migrants who are healthier on average appear to be selected for migration to rural counties while migrants do not appear to be selected to urban counties. Therefore, we have **partially confirmed Hypothesis 4**: health-related selection for migration improves our ability to explain county-level premature mortality in rural counties but not in urban counties.
...
```{r urb rural comparison plot, include = FALSE, echo = FALSE, warning = FALSE}
