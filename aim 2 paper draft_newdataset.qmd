...
```{r justify exclusion of covid years, echo = FALSE, include = FALSE}
...
1.  County-to-county migration patterns improve the explainability of autoregressive models of county-level mortality. <!--# 2. The total number of years that a county has had positive net migration or negative net migration improves the explainability of autoregressive models of county-level mortality. -->

2.  County-to-county migration patterns improve the explainability of spatio-temporal autoregressive models of county-level health.

3.  The role that intercounty migration <!--# and net migration -->plays in county-level mortality differs significantly between rural and urban counties.
...
```{r plot of mig_it relative to y_i,t-1 , echo = FALSE}
...
load("data_processed/migterm_imp.Rdata") #called migterm_imp ... reloading to make sure nothing has changed 
...
# ggplot(data = migurb) + geom_line(aes(x = year, y = migmort, group = destid, color = chrr_urb_code))

ggplot(data = groupeddata) + geom_line(aes(x = year, y = meddiff, group = factor(code2013), color = factor(code2013))) + 
  scale_color_discrete(labels = c("Large Central Metro", "Large Fringe Metro", "Medium Metro", "Small Metro", "Micropolitan", "Noncore"), name = "") +
  xlab("Year") + ylab("Median difference between migration weighted\n average and destination premature mortality rate") + geom_hline(yintercept = 0, color = "black") + theme_bw() 

```

Because we know county-level health to be related to county-level natality, and because our outcome variable (premature age-adjusted mortality) may not capture the health of young people in counties, we first conducted a sensitivity analysis of the effect of natality on the effect of our migration term. We determined that natality does not change the effect or explainability of the migration term and therefore does not need to be included. <!--# more here??? -->
...
```{r spatial stuff, echo = FALSE, warning = FALSE, message=FALSE}
...
```{r function for AIC BIC of splm models, echo = FALSE}
...
```{r imputation for spatial stuff - no longer in use , echo = FALSE, include = FALSE}
#select only the cols needed for modelling (can easily add ft back in later)
# mnnmice = mnn %>% select(destid, migterm, rate_d0, rate_d1, t, rural)
# 
# #https://github.com/amices/mice/issues/384
# 
# timeupdated = c("migterm", "rate_d0", "rate_d1")
# timeinvariant = c("rural", "destid")
# 
# #---- predictor matrix ----
# blocks <- timeupdated
# predict <- matrix(1, length(blocks), ncol = ncol(mnnmice)) 
# rownames(predict) = blocks 
# colnames(predict) = colnames(mnnmice)
# 
...
# predict[timeupdated, timeupdated] <- 
#   (diag(x = 1, nrow = length(timeupdated), 
#         ncol = length(timeupdated)) == 0)*1

#---- impute ----
#this crashes due to a singularity issue - which makes sense because we don't actually have a model..... 
# data_imputed <- mice::mice(data = mnnmice, 
#                      m = 2, maxit = 5, method = "2l.lmer", 
#                      predictorMatrix = predict, where = is.na(mnnmice), 
#                      blocks = as.list(rownames(predict)), 
#                      seed = 20210126)
# #


# not sure of the state of what's below here.... bits n pieces of several attempts 
# #referencing this stackoverflow https://stackoverflow.com/questions/75994509/multiple-imputation-for-longitudinal-data-with-2l-pan-or-panimpute-mice-package 
# 
# predmat = mice::quickpred(mnnmice, exclude = c("year", "ft", "t"), mincor = 0.1)
# predmat["destid",] = 2
# #imp = mice::mice(mnnmice, predictorMatrix = predmat, m = 10, maxit = 10, method = "pmm") #remove.collinear = FALSE)
# 
# imp = mice::mice(data = mnnmice, m=3, maxit = 5, method = "2l.lmer", predictorMatrix = predmat)
# 
# 
# formulaimp = list(rate_d0 + migterm ~ (1|destid))
# predmat["destid",]= -2
# imp = mice::mice(mnnmice, predictorMatrix = predmat, method = "2l.pan", formulas = formulaimp, m = 10, maxit = 10), method = "2l.pan") 
# 
# 
# imputetemp = mnnmice %>% mice(m=10, method = "pmm", ridge = 0.001, threshold = 1.1)
# 
# 
# cp_all = tidycensus::get_acs(geography = "county", year = 2019, variables = c(tpop = "B01003_001"), survey = "acs5", output = "wide", geometry = TRUE)
# 
# library(mice)
# 
# cpmnn_all = merge(cp_all, mnn, by.x = "GEOID", by.y = "destid")
# cpmnn_all = cpmnn_all %>% select(-c("NAME", "tpopE", "tpopM","ctyname", "st_abbrev"))
# 
# predmat = mice::quickpred(cpmnn_all, exclude = c("lagyear", "origid", "GEOID", "cyear", "year","migdiff", "ft", "geometry", "statecode", "countycode", "code2013", "chrr_urb_code", "rural", "migterm_nat"), mincor = 0.1)

```

```{r interpolation for spatial stuff - no longer in use, include = FALSE, echo = FALSE}
# mnnall = mnnmice %>% rowid_to_column() %>% 
#   complete(t = c(1:8)) %>% 
#   group_by(destid) %>% 
#   mutate(migtermapprox = zoo::na.approx(migterm), 
#          rate_d0approx = zoo::na.approx(rate_d0),
#          rate_d1approx = zoo::na.approx(rate_d1))
#            
# temp = mnnmice %>%  group_by(destid) %>% 
#   mutate_at(vars(matches("migterm")), list(interp = zoo::na.approx), na.rm = FALSE)
# 
# 
# temp = mnnmice %>%  group_by(destid) %>% 
#   mutate_at(vars(matches("rate", "migterm")), list(interp = zoo::na.approx), na.rm = FALSE)
# 
# 
# temp = mnnmice %>%  group_by(destid) %>% 
#   mutate(mig_int = zoo::na.approx(migterm))
# 
# 
# 
# = zoo::na.approx(migterm), 
#          rate_d0approx = zoo::na.approx(rate_d0),
#          rate_d1approx = zoo::na.approx(rate_d1))
#   
#            
...
# mnnall = ts(mnnall)
# 
# mnni = mnnall %>% group_by(destid) %>% 
#   interpTs(type = "linear")
#   
#   
#   model(naive = ARIMA(migterm ~ -1 + pdq(0,1,0) + PDQ(0,0,0))) %>%
#   interpolate(mnnall)
# 
# 
# data.frame(t = seq(mnnmice$t[1], max(mnnmice$t), by = 1)) %>% 
#   full_join(mnnmice, by = "t") %>% 
#   mutate(approx = na.approx(migterm))
# 
# library(tsibble)
# temp = mnnmice %>% as_tsibble(index = t) %>% fill_gaps()
```
...
To justify further investigation, first we determined that our migration term added explainability (as measured by low BIC score) to our most basic model. These results are shown below.

`r basicmodelsummaries`

After concluding that the migration term did add explainability to our model, we then tested for splines. The best fitting model in terms of BIC score included four spline terms for both the autoregressive term and our weighted average migration term. This model will be called $model2A$. Thus, models that account for migration had more explanatory power than models that included only the prior year's average county-level mortality rate. This confirms Hypothesis 2A. model2A is shown below.
...
```{r addition of urb for aim 2b i believe this section can be deleted since we decided to just create separate datasets, include = FALSE, message = FALSE, warning = FALSE}
# m1_urb = lme4::lmer(rate_d1 ~ ft + rate_d0 + (1+t|destid) + migterm + factor(urbcode), 
#                     data = mnn,
#                     control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
# 
# m1_netmig = lme4::lmer(rate_d1 ~ft + rate_d0 + (1+t|destid) + migterm + netmig, data = mnn, 
#                      control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
# 
# m1_netmigi = lme4::lmer(rate_d1 ~ft + rate_d0 + (1+t|destid) + migterm + migterm*netmig + netmig, data = mnn, 
#                      control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
# 
# m1_urbi = lme4::lmer(rate_d1 ~ ft + rate_d0 + (1+t|destid) + migterm + factor(urbcode) + migterm*factor(urbcode), 
#                     data = mnn,
#                     control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
# m1_urbmig = lme4::lmer(rate_d1 ~ ft + rate_d0 + (1+t|destid) + migterm + factor(urbcode) + netmig, 
#                     data = mnn,
#                     control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
# 
# 
# m1_urbmorti = lme4::lmer(rate_d1 ~ ft + rate_d0 + (1+t|destid) + migterm + factor(urbcode) + rate_d0*factor(urbcode), 
#                     data = mnn,
#                     control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
# 
# 
# m1_urbmortmigi = lme4::lmer(rate_d1 ~ ft + rate_d0 + (1+t|destid) + migterm + factor(urbcode) + rate_d0*factor(urbcode) +migterm*factor(urbcode), 
#                     data = mnn,
#                     control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
# 
# m1_spline55urb = lme4::lmer(rate_d1 ~ ft + ns(rate_d0,df = 5) + (1+t|destid) + ns(migterm,df = 5)*factor(urbcode), data = mnn,
#                 control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
# 
# # m1_spline55urbmort = lme4::lmer(rate_d1 ~ ft + ns(rate_d0,df = 5)*factor(urbcode) + (1+t|destid) + ns(migterm,df = 5)*factor(urbcode), data = mnn,
# #                 control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
...
##### [Pitfalls and Alternatives:]{.underline}
...
Second, we selected our outcome of interest, county-level premature age-adjusted mortality, because it is comparable across years and locations and because it is consistently and robustly defined. However, in 2019, the median age of all movers across the US was under 30 years old [@s0701:g] - therefore, even when accounting for premature mortality (deahts occuring in individuals under age 75), the population that is migrating is unlikely to be the same population that is dying. Subsequently, trends in county-level premature mortality might be better explained by trends in county-level migration twenty to fifty years earlier. That said, county-level measures of premature mortality are commonly used as a proxy for county overall health. For instance, a 1983 report by the WHO recommending measures in mortality and morbidity to understand the relationship between health and migration [@gushulak2006], and measures of length of life are allocated 50% weight in the County Health Rankings Model of place-based health outcomes [@explore]. Ideally we would use a more precise age-group-specific measure of mortality paired with an age-group-specific measure of migration; however, this would require a migration data source other than the IRS migration flow data which we use here. Additionally, ideally, we would confirm our findings by assessing the health of migrants themselves. This task would require individual longitudinal data. This is not currently within the scope of this project.
